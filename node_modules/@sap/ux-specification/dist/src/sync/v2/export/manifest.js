    ...
    if (configPage.navigation) {
        for (const association in configPage.navigation) {
            ...
            if (manifestPage['pages'] && !Object.values(manifestPage['pages'])[0]) {
                manifestPage['pages'][`${configPages[association].pageType}|${configPages[association].entitySet}`] = getNewPageTemplate(configPages[association].pageType);
            }
            ...
            if (manifestPage['pages']) {
                Object.keys(manifestPage['pages']).forEach((key) => {
                    ...
                    if (currentPage.component.name === 'sap.suite.ui.generic.template.ObjectPage') {
                        const configKey = currentPage.navigationProperty
                            ? `ObjectPage_${currentPage.entitySet}_${currentPage.navigationProperty}`
                            : `ObjectPage_${currentPage.entitySet}`;
