import { ObjectPageResponsiveTable, ObjectPageAnalyticalTable, ObjectPageGridTable, ObjectPageTreeTable, ObjectPageForm } from './index';
import { ViewTypes, SAPUI5_FRAGMENT_CLASS, SAPUI5_VIEW_CLASS } from '../../common/webapp/manifest/sapUi5';
/**
 * Section
 * @isViewNode true
 */
export interface ObjectPageSection {
    /**
     * Whether the section should be visible on the screen.
     */
    visible?: boolean;
}
/**
 * Form
 * @isViewNode true
 */
export interface ObjectPageSectionFormV2 extends ObjectPageSection {
    form?: ObjectPageForm;
}
/**
 * Table
 * @isViewNode true
 */
export interface ObjectPageSectionTableV2 extends ObjectPageSection {
    table?: ObjectPageResponsiveTable | ObjectPageAnalyticalTable | ObjectPageGridTable | ObjectPageTreeTable;
}
/**
 * Subsection
 * @isViewNode true
 */
export interface ObjectPageSubSectionV2 extends ObjectPageSection {
    subsections?: object;
}
export interface GenericSections {
    [key: string]: ObjectPageSectionTableV2 | ObjectPageSectionFormV2 | ObjectPageSubSectionV2;
}
/**
 * Custom Sections
 * @isViewNode true
 */
export interface CustomSections {
    custom?: (ObjectPageCustomSectionView | ObjectPageCustomSectionFragment)[];
}
export interface ObjectPageCustomSectionBase {
    relatedFacet: string;
    relativePosition: SectionPosition;
    /**
     * The label of the custom section, preferably as an i18n key
     * @i18nClassification TIT: Custom section title
     */
    title: string;
}
/**
 * View
 * @isViewNode true
 */
export interface ObjectPageCustomSectionView extends ObjectPageCustomSectionBase {
    className: typeof SAPUI5_VIEW_CLASS;
    viewName: string;
    type: ViewTypes;
}
/**
 * Fragment
 * @isViewNode true
 */
export interface ObjectPageCustomSectionFragment extends ObjectPageCustomSectionBase {
    className: typeof SAPUI5_FRAGMENT_CLASS;
    fragmentName: string;
    type: ExtensionFragmentTypes;
}
export declare enum SectionPosition {
    AfterFacet = "AfterFacet",
    BeforeFacet = "BeforeFacet",
    ReplaceFacet = "ReplaceFacet"
}
export declare enum ExtensionFragmentTypes {
    XML = "XML"
}
